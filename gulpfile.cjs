...
// Directory containing the coverage report.
const coverageDir = "./coverage"

// Path to the
const rescriptOutDir = "./lib/bs/"
...
//==============================================================================
// Generate `.resi` interface definition files from the sources.
function generateResiDir(rescriptOut, dir) {
    return src(`${rescriptOut}/${dir}/**/*.cmi`)
        .pipe(gExec((file) => `bsc -i ${file.path}`, { pipeStdout: true }))
        .pipe(
            grename((path) => {
                path.extname = ".resi"
            })
...
function generateResi() {
    return generateResiDir(rescriptOutDir, srcDir)
}
...
function processSW() {
    return processJS(outDir + "/" + serviceWorkerJS, serviceWorkerJS)
}

function processApp() {
    return processJS(srcDir + "/" + appMJS, appJS)
}

//==============================================================================
// Watch for changes
function watchSource(cb) {
    watch("./src/**/*.mjs", { ignoreInitial: false }, bundleTarget)
    cb()
}
...
function cleanCoverage(cb) {
    delDirectory(coverageDir, cb)
    cb()
}

async function cleanRescript() {
    return (
...
const cleanTarget = parallel(cleanHTTP, cleanRescript, cleanCoverage)

const bundleTarget = series(
    parallel(copyAssets, runTailwind),
    parallel(
...
const resiTarget = series(generateResi)

// eslint-disable-next-line no-undef
exports.resi = resiTarget
